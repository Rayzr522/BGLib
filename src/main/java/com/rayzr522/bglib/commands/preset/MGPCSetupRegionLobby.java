...
@CommandInfo(name = "region lobby", usage = "/{command} setup region lobby", desc = "Set the lobby region for the selected arena", pattern = "region l(obby)?", perm = "{base}.setup.region.lobby")
public class MGPCSetupRegionLobby implements BitzCommand {
	...
	public boolean execute(CommandSender sender, String[] args, BitzPlugin plugin) {
		...
		arena.setLobbyRegion(partial.toRegion());
		plugin.messenger.playerMessage(sender, MGMessages.LOBBY_REGION_SET.msg, arena.getName());
